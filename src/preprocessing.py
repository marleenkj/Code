...
def get_country_name(df):
    ...
    for i in range(len(code_unique)):
        dict_country_code[code_unique[i]] = coco.convert(names=code_unique[i], to='name_short', not_found=None)
...
def data_preprocessing_bridge(df, dict_names: dict, df_ig_shipper):
    ...
    shipper_ig = ['00' + number for number in df_ig_shipper['Vendor'].astype(str).to_list()]
    df['IG'] = np.where(df['Carrier code'].isin(shipper_ig), 1, 0)

    # Correct weight column
    df['Sender weight (kg)'] = np.where(df['Weight unit']=='LB', df['Sender weight']*0.45359237, df['Sender weight'])
    df['Sender weight (kg)'] = np.where(df['Weight unit']=='G', df['Sender weight']/1000, df['Sender weight'])
    df = df.drop(columns = ['Weight unit', 'Sender weight'])

    # Correct volume column
    df['Volume (m3)'] = np.where(df['Volume unit']=='CMD', df['Volume']/1000, df['Volume'])
...
def merge_for_app(df, df_shipper, df_receiver, df_distances):
    ...
    data = df.merge(df_shipper, how='left', on=['DC code'], suffixes=(None, '_shipper'))
    ...
    # Merging df_receiver on full nshift data
    data = data.merge(df_distances, how='inner', on=[
        'Receiver longitude', 'Receiver latitude', 'Shipper longitude', 'Shipper latitude',])
