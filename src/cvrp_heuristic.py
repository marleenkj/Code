...
def find_nearest_customer(data, current_location, remaining_capacity, remaining_customers):
...
def construct_initial_solution(data, capacity_limit):
    ...
    while remaining_customers:
        nearest_customer = find_nearest_customer(data, current_location, remaining_capacity, remaining_customers)
...
def evaluate_route(data, route):
    ...
    for i in range(len(route) - 1):
        distance_arc = data["distance_matrix"][data["customers"].index(route[i])][data["customers"].index(route[i+1])] #in meters
        route_distance += distance_arc
        current_load -= data["demands"][data["customers"].index(route[i])] # in kg
        ...
        for r in range(len(solution)):
            ...
            for i in range(1, len(route) - 1):
                for j in range(i + 1, len(route)):
                    ...
                    new_cost = evaluate_solution(solution) - evaluate_route(route) + evaluate_route(new_route)
...
def swap_customers(routes):
    ...
    if sum(customer1[2] for customer1 in route2) - customer2[2] + customer1[2] <= capacity:
...
def evaluate_new_solution(list_solution, terminals):
    def evaluate_road(list_solution, terminals):
        ...
        dict_co2_road = pd.read_csv("road_emission.csv", index_col = 0).T.to_dict("index")["emissions"]
        for i in terminals:
            if f"T{i}" == closest_dct:
                data = create_data_model(dict(((key, dict_points[key]) for key in [dc_name] + list_solution[i])), 10)
            else:
                data = create_data_model(dict(((key, dict_points[key]) for key in [f"T{i}"] + list_solution[i])), 10)
        ...
        pd.DataFrame.from_dict(dict_co2_road, orient = "index").rename({0:"emissions"}, axis = 1).reset_index().to_csv("road_emission.csv", index = False)
    ...
    def evaluate_rail(list_solution):
        ...
        for i in range(len(list_solution)):
            if f"T{i}" != closest_dct:
                total_demand = sum([int(item[2]) for item in list(dict(((key, dict_points[key]) for key in list_solution[i])).values())])
