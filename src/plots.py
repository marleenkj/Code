...
def graph_solution_direct(x, dict_points, dict_terminals, df_distance_matrix, path_plot = False):
    ...
    dc_coords = [dict_points[dc][1],dict_points[dc][0]] #lon/lat [dict_points[node][::-1] for node in routes_terminal[j]]
    ...
    fig = go.Figure()
    waypoints = get_waypoints_osrm([dc_coords,c_coords])["geometry"]["coordinates"]
    lat = [coords[1] for coords in waypoints]
    lon = [coords[0] for coords in waypoints]
    fig.add_trace(go.Scattermapbox(lat=lat, lon=lon, mode='lines', name = "road", marker={'size': 10}))
    if terminal != closest_dct:
        # Add all routes
        waypoints = get_waypoints_osrm([dc_coords,dct_coords])["geometry"]["coordinates"]
        lat = [coords[1] for coords in waypoints]
        lon = [coords[0] for coords in waypoints]
        waypoints = get_waypoints_osrm([t_coords,c_coords])["geometry"]["coordinates"]
        lat = lat + [coords[1] for coords in waypoints]
        lon = lon + [coords[0] for coords in waypoints]
        fig.add_trace(go.Scattermapbox(lat=lat, lon=lon, mode='lines', name = "railroad", marker={'size': 10}))
    else:
        pass
    fig.add_trace(go.Scattermapbox(lat=[dc_coords[1]], lon=[dc_coords[0]], mode='markers', name = dc, marker={'size': 10}))
    fig.add_trace(go.Scattermapbox(lat=[c_coords[1]], lon=[c_coords[0]], mode='markers', name = client, marker={'size': 10}))
    fig.add_trace(go.Scattermapbox(lat=[t_coords[1]], lon=[t_coords[0]], mode='markers', name = terminal, marker={'size': 10}))
    fig.add_trace(go.Scattermapbox(lat=[dct_coords[1]], lon=[dct_coords[0]], mode='markers', name = closest_dct, marker={'size': 10}))
...
def graph_road(df, list_routes, path_plot = False):
    ...
    #Add clients
    list_clients = pd.Series(itertools.chain(*list_routes)).drop_duplicates()[1:]
    fig.add_trace(go.Scattermapbox(
        lat=[dict_points[i][0] for i in list_clients], 
        lon=[dict_points[i][1] for i in list_clients], 
        name="Clients", mode='markers', customdata=list_clients, 
        hovertemplate='''<br>%{customdata}<br>''', marker={'size': 10, "color": "orange"}))
    
    #Add DC
    fig.add_trace(go.Scattermapbox(lat=[list(dict_points.values())[0][0]], lon=[list(
        dict_points.values())[0][1]], mode='markers', name="DC", marker={'size': 10, 'color': "red"}))
...
def graph_solution(df, dict_routes, terminal_allocation, dict_terminals, closest_dct, path_plot = False):
    ...
    #Add clients
    list_clients = pd.Series(itertools.chain(*terminal_allocation)).drop_duplicates()
    ...
    for i in dict_routes.keys():
        if dict_routes[i] != [[]]:
            fig.add_trace(go.Scattermapbox(lat=[dict_terminals[i][0]], lon=[
                          dict_terminals[i][1]], name=f"Terminal {i[-1]}", mode='markers', marker={'size': 10, "color": "blue"}))
    fig.update_layout(
        legend=legend_layout,
        margin={"r": 0, "t": 40, "l": 0, "b": 40},
        mapbox={'zoom': 4, 
                "center": {'lat': df["Receiver latitude"].mean(),'lon': df["Receiver longitude"].mean()}}
                )
    fig.update_layout(mapbox_style="open-street-map")
    if path_plot == False:
        return fig
...
def graph_solution_with_radius(df, dict_routes, terminal_allocation, dict_terminals, closest_dct, list_terminals, path_plot = False):
    #dict_routes = dict_results["routes railroad"]
    dict_points = create_dict_points(
        df, "Shipper name", "Shipper latitude", "Shipper longitude")
    dict_points.update(create_dict_points(
    ...
    for i in dict_routes.keys():
        if dict_routes[i] != [[]]:
            fig.add_trace(go.Scattermapbox(lat=[dict_terminals[i][0]], lon=[
                          dict_terminals[i][1]], name=i, mode='markers', marker={'size': 10, "color": "blue"}))
    fig.update_layout(
        legend=legend_layout,
        margin={"r": 0, "t": 40, "l": 0, "b": 40},
        mapbox={'zoom': 4, 
...
def graph_solution_with_radius_old(df, dict_results, dict_terminals, closest_dct):
    dict_routes = dict_results["routes railroad"]
        ...
        for j in range(len(routes_terminal)):
            if routes_terminal[j]:
                ...
                if i != closest_dct:
                    ...
                    lat = [coords[1] for coords in waypoints]+lat
                    lon = [coords[0] for coords in waypoints]+lon
                    fig.add_trace(go.Scattermapbox(lat=lat, lon=lon, name=f"Railroad {j+1} over {routes_terminal[j][0]}", mode='lines', marker={
                                  'size': 10, "color": color[list(dict_terminals.keys()).index(i)]}))
                else:
                    fig.add_trace(go.Scattermapbox(lat=lat, lon=lon, name=f"Truck {j+1} directly from {routes_terminal[j][0]}", mode='lines', marker={
                                  'size': 10, "color": color[list(dict_terminals.keys()).index(i)]}))
    fig.add_trace(go.Scattermapbox(lat=[list(dict_points.values())[0][0]], lon=[list(
        dict_points.values())[0][1]], mode='markers', name="DC", marker={'size': 10, 'color': "red"}))
    df = df[df["Receiver name"].isin(
    ...
    dict_clients = create_dict_points(
        df, "Receiver name", "Receiver latitude", "Receiver longitude")
    fig.add_trace(go.Scattermapbox(lat=[i[0] for i in list(dict_clients.values())], lon=[i[1] for i in list(dict_clients.values(
    ))], name="Clients", mode='markers', customdata=list(dict_clients.keys()), hovertemplate='''<br>%{customdata}<br>''', marker={'size': 10, "color": "orange"}))
    for i in dict_routes.keys():
        if dict_routes[i] != [[]]:
            fig.add_trace(go.Scattermapbox(lat=[dict_terminals[i][0]], lon=[
                          dict_terminals[i][1]], name=i, mode='markers', marker={'size': 10, "color": "blue"}))
            ...
            fig.update_layout(mapbox={
                ...
                "layers": [
                    {
                        "source": json.loads(poi_poly(None, poi=poi, radius=150000).to_json()),
                        "below": "traces",
                        "type": "line",
                        "color": "green",
                        "line": {"width": 1.5},
