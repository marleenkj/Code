...
from src.evaluation_unimodal import evaluate_cvrp, evaluate_direct
...
def preprocessing_modelling_without_grouping(df, date_from, date_to, truck_capacity, volume):
    df = df[(df["Delivery date"]>=pd.to_datetime(date_from))&(df["Delivery date"]<=pd.to_datetime(date_to))]
    df["Sender weight (kg)"] = df["Sender weight (kg)"]*volume
    df = df[(df["Sender weight (kg)"]>1)&(df["Sender weight (kg)"]<truck_capacity)]
    date_min = df["Delivery date"].min()
...
def co2_modell(df, df_distance_matrix, dict_terminals, date_from, date_to, 
    ...
    """  
    # Preprocessing
    df, date_min, date_max = preprocessing_modelling(df, date_from, date_to, truck_capacity, volume)
    ...
    if df.shape[0] == 0:
        ...
        for i in range(len(routes_names_truck)):
            ...
            for j in range(1,len(routes_names_truck[i])):
                if routes_names_truck[i][j] != routes_names_truck[i][j-1]:
                    route_string = route_string+"-"+routes_names_truck[i][j]
            new_record = pd.DataFrame(
                [['Unimodal road', f"Truck {i+1}", 
                  route_string, routes_load[i], distance_truck[i], co2_truck[i]]],
                columns=columns_df_results_details)
            df_results_truck = pd.concat([df_results_truck, new_record], ignore_index=True)
        ...
        if mode == "multi":
            ...
            if algorithm == "base":
                ...
                list_solution, closest_dct = create_solution_individual_analysis(df, dict_terminals, dict_points)
                #logger.info(list_solution)
...
def co2_evaluation(df, df_distance_matrix, dict_terminals, date_from, date_to, 
               algorithm = "base", volume = 1, power = "electric", country = "france", 
               truck_capacity = 13810, nb_trucks = 40):
    """
    ...
    if algorithm == "base":
        co2_railroad, distance_railroad, dict_routes = evaluate_solution(df, list_solution, dict_terminals, 
                                                                         closest_dct, df_distance_matrix, power, country)
...
def co2_rail(df, df_distance_matrix, dict_terminals, date_from, date_to, 
    ...
    """
    df = preprocessing_modelling(df, date_from, date_to, truck_capacity, volume)
    if df.shape[0] == 0:
        print(f"No deliveries from {date_from} to {date_to}")
        return None
    else:
...
def co2_modell_direct(df, df_distance_matrix, dict_terminals, date_from, date_to, 
               algorithm = "base", volume = 1, power = "electric", country = "france", 
               truck_capacity = 13810, nb_trucks = 40, mode = "combined"):
    ...
    """  
    # Preprocessing
    df, date_min, date_max = preprocessing_modelling_without_grouping(df, date_from, date_to, truck_capacity, volume)
    ...
    if df.shape[0] == 0:
        ...
        if total_load > truck_capacity:
            ...
            try: 
                ...
                co2_road, distance_road = evaluate_cvrp(data, routes_index_road)
            except:
        ...
        dict_results = {"co2 road": co2_road, 
                        ...
                        "haversine distance": get_haversine_distance_latlon(dict_points[dc], dict_points[client]),
