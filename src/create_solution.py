...
def create_solution_individual_analysis(df, dict_terminals, dict_points):
    ...
    df_individual["terminal allocation"] = np.where(
        df_individual["Recommendation"] == "Road", closest_dct, df_individual["terminal allocation"])
    df_individual["terminal allocation"] = np.where(
        (df_individual["distance endhaul"] > 150)&(df_individual["distance endhaul"] > df_individual["distance railroad"]*0.2), closest_dct, df_individual["terminal allocation"])
...
def create_solution(df, dict_terminals, dict_points):
    ...
    # Test 150km Rule:
    distances["More than 150km"] = np.where(distances["Dmin"] > 150000, "Yes", "No")
    ...
    distances = distances.reset_index().merge(df_distances[df_distances["id_x"] == closest_dct][["Dminidx","Distance"]], 
                                              on=["Dminidx"], how = "left").set_index('index')
    distances["Distance"] = distances["Distance"].fillna(0)
    distances["Travel distance"] = distances["Dmin"] + distances["Distance"] + closest_dct_distance
    distances["More than 20%"] = np.where(distances["Dmin"] > distances["Travel distance"]*0.2, "Yes", "No")
    distances["More than 20%"] = np.where(distances["Dminidx"] == closest_dct, "Road", distances["More than 20%"])

    # 150km Rule and 20% Rule:
    distances["Dminidx"] = np.where((distances["Dmin"] > 150000)&(distances["Dmin"] > distances["Travel distance"]*0.2), 
                                    closest_dct, distances["Dminidx"])
...
def sensi_distance(df, dict_terminals, df_distance_matrix, dc):
    ...
    dict_solution = {}
    for i in dict_terminals.keys():
        dict_solution[i] = distances[distances["Dminidx"]==i].reset_index()["index"].to_list()
        list_solution.append(distances[distances["Dminidx"]==i].reset_index()["index"].to_list())
        average_distance.append(distances[distances["Dminidx"]==i]["Dmin"].mean())
    dict_main_terminals = {}
    for i in range(len(list_solution)):
        weight = df[df["Receiver name"].isin(list_solution[i])]["Sender weight (kg)"].sum()/1000
...
def create_dict_terminals(df_terminal):
    ...
    dict_terminals = df_terminal[["longitude","latitude","demand"]].T.to_dict("list")
...
def get_closest_dct(df_test, dict_terminals):
    ...
    point_dc = str(df_test["Shipper longitude"].iloc[0])+','+str(df_test["Shipper latitude"].iloc[0])
    ...
    closest_dct = list(dict_terminals.keys())[distances.index(np.min(distances))]
    return closest_dct

def get_points_terminal(df):
    df_points = df[["Receiver name", 'Receiver longitude', 'Receiver latitude']]
    df_points.loc[-1] = df[["Terminal id",'Terminal longitude','Terminal latitude']].iloc[1].to_list()  # adding a row
...
def initial_solution(df_test, df_terminal):
    ...
    df_test['key'] = 1
    df_terminal["key"]=1
    df_terminal_receiver = pd.merge(df_test, df_terminal, on='key').drop("key", 1).rename(
        columns={"latitude": "Terminal latitude", "longitude": "Terminal longitude"})
    ...
    for i in df_terminal["id"].to_list():
        list_solution.append(df_tc[df_tc["Terminal id"]==i]["Receiver name"].to_list())
...
def create_solution_old(df, dict_terminals, closest_dct):
    ...
    distances = create_distance_matrix(dict_points)[list(df["Receiver name"].unique())]
    distances = distances[~distances.index.isin(list(df["Receiver name"].unique()))]
    distances = distances.T
    distances['Dminidx'] = distances.idxmin(axis=1)
    distances['Dmin'] = distances.min(axis=1)
    #print(dc)
    distances["Dminidx"] = np.where(distances["Dminidx"]==dc, closest_dct, distances["Dminidx"])
