...
def evaluate_prehaul_multi(
    ...
    list_customers = df[df["Receiver name"].isin(
        list_solution_flattened)]["Receiver name"].to_list()
    list_weights = df[df["Receiver name"].isin(
        list_solution_flattened)]["Sender weight (kg)"].to_list()

    df_t = df[df["Receiver name"].isin(list_solution_flattened)]
    data_t = create_data_model(
        df_t,
        df_distance_matrix,
        dict(
            ((key,
              dict_points[key]) for key in [dc] +
             list_customers)),
        nb_trucks,
        truck_capacity)
    ...
    for i in range(len(routes_names)):
        ...
        new_record = pd.DataFrame([['Railroad',
                                    f"Prehaul Container {i+1}",
                                    route_string,
                                    routes_load[i],
                                    distance_prehaul,
                                    co2]],
...
def evaluate_multi_premainhaul(
    ...
    list_solution_dc, closest_dct = create_solution_150_20(
    ...
    list_solution_of_closest_dct = list_solution_dc[list(
        dict_terminals.keys()).index(closest_dct)]
    ...
    for i in range(len(routes_names)):
        ...
        new_record = pd.DataFrame([['Road',
                                    ...
                                    co2_allroad[i]]],
                                  columns=columns_df_results_details)
        df_results_allroad = pd.concat(
            [df_results_allroad, new_record], ignore_index=True)
    ...
    for i in range(len(list_solution_new)):
        if list_solution_new[i]:
            list_customers = df[df["Receiver name"].isin(
                list_solution_new[i])]["Receiver name"].to_list()
            list_weights = df[df["Receiver name"].isin(
                list_solution_new[i])]["Sender weight (kg)"].to_list()
            trucks_and_clients = knapsack(
                list_customers, list_weights, truck_capacity)
            weights_container = []
            for container in trucks_and_clients:
                load = df[df["Receiver name"].isin(
                    container)]["Sender weight (kg)"].sum()
    ...
    df_results = pd.concat(
        [df_results_allroad, df_results_rail, df_results_prehaul], ignore_index=True)
...
def cvrp_routing(
    ...
    co2_endhaul, distance_endhaul = evaluate_cvrp(
        data, routes_index, details="individually")
    return co2_endhaul, distance_endhaul, routes_names, routes_load


def evaluate_endhaul_multi(
        df_combined,
        solution_terminal,
        dict_terminals,
        terminal,
...
def co2_modell_multi(
    ...
    df_combined, date_min, date_max = preprocessing_modelling(
        df_combined, date_from, date_to, truck_capacity, volume)
    # Allocation to terminals + Evaluate prehaul, allroad, and mainhaul
    # seperately
    ...
    for dc in dcs:
        ...
        if df.shape[0] == 0:
            ...
            df_results["DC"] = dc
            df_results_premainhaul = pd.concat(
                [df_results_premainhaul, df_results], ignore_index=True)
            co2 += co2_dc
    ...
    for i in range(len(list_rail)):
        if list_rail[i]:
            ...
            df_results_endhaul_all = pd.concat(
                [df_results_endhaul_all, df_results_endhaul])
    ...
    co2 += co2_endhaul_all
    df_results = pd.concat(
        [df_results_endhaul_all, df_results_premainhaul], ignore_index=True)
...
def evaluate_solution_multi(
        ...
        if list_solution[i]:
            ...
            if terminal != closest_dct:
                ...
                data_t = create_data_model(
                    ...
                    dict(
                        ((key,
                          dict_points[key]) for key in [terminal] +
                            solution_terminal)),
                    nb_trucks,
                    truck_capacity)
                routes_index, routes_names, routes_load = cvrp_ortools(data_t)
                co2, distance = evaluate_cvrp(
                    data_t, routes_index, details="individually")
                ...
                for i in range(len(routes_names)):
                    ...
                    new_record = pd.DataFrame(
                        ...
                        columns=columns_df_results_details)
                    df_results = pd.concat(
                        [df_results, new_record], ignore_index=True)
                co2 = sum(co2)
    ...
    df_results.loc["total"] = df_results[[
        "Distance", "GHG Emissions"]].sum(axis=0).to_dict()
