...
try:
    df_distances_rail = pd.read_excel(
        "../data/external/intermodal_terminals/rne_terminals_distances.xlsx")
except BaseException:
    df_distances_rail = pd.read_excel(
        "../../data/external/intermodal_terminals/rne_terminals_distances.xlsx")
df_distances_rail["id_x"] = "T" + \
    (df_distances_rail[f"facilityId_x"] + 1).astype(str)
df_distances_rail["id_y"] = "T" + \
    (df_distances_rail[f"facilityId_y"] + 1).astype(str)
...
def evaluate_rail_for_solution_i(
    ...
    distance_arc = df_distances_rail[(df_distances_rail["id_x"] == closest_dct) & (
        df_distances_rail["id_y"] == terminal)]["Distance"].iloc[0] * 1000
        ...
        if list_solution[i]:
            if terminal != closest_dct:
                ...
                for i in range(len(routes_names)):
                    ...
                    distance_dc_closest_dct = math.ceil(
                        df_distance_matrix.loc[dc][closest_dct])
                    co2_dc_closest_dct = co2_truck(
                        container_load, distance_dc_closest_dct)
...
def evaluate_all_road(
    ...
    if list_solution:
        ...
        dict_t_points = create_dict_points(
            df_t, "Shipper name", "Shipper latitude", "Shipper longitude")
        dict_t_points.update(
            create_dict_points(
                df_t,
                "Receiver name",
                "Receiver latitude",
...
def evaluate_endhaul_direct(
    ...
    for i in range(len(list_solution)):
        ...
        data_t = create_data_model(df_t, df_distance_matrix, dict(((key, dict_points[key]) for key in [
                                   terminal] + [list_solution[i]])), nb_trucks, truck_capacity)
        if sum(data_t["demands"]) > truck_capacity:
            # besser wäre es ein knapsack problem zu lösen
            routes_index, routes_names, routes_load = cvrp_ortools(data_t)
            co2_endhaul_i, distance_endhaul_i = evaluate_cvrp(
                data_t, routes_index)
        else:
            ...
            co2_endhaul_i = co2_truck(
                sum(data_t["demands"]), distance_endhaul_i)
...
def evaluate_solution_drop(
    ...
    Drop algorithm
    """
    list_solution_of_closest_dct = list_solution[list(
    ...
    for i in range(len(list_solution)):
        if list_solution[i]:
            ...
            if closest_dct != terminal:
                ...
                dict_with_train[terminal] = {
                    "co2": co2_with_train,
                    "distance": distance_with_train,
                    "routes": routes_names_train}
            ...
            if list_solution[i]:
                ...
                if closest_dct != terminal:
                    ...
                    if co2_without_train < co2_dc_with_train:
                        ...
                        rail_to_road_switch = True
                        list_solution_of_closest_dct = list_solution_of_closest_dct + \
                            list_solution[i]
    ...
    if not at_least_one_time_road_better:
        co2_total += co2_dc
        distance_total += distance_dc
    if sum(list(dict_co2_old.values())) < sum(list(dict_co2.values())):
        co2_total = sum(list(dict_co2_old.values()))
    list_solution[list(dict_terminals.keys()).index(
        closest_dct)] = list_solution_of_closest_dct
