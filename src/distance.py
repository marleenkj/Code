...
def get_distance_osrm(a, b) -> float:
    ...
    # by default you get only one alternative so you access 0-th element of
    # the `routes`
...
def distance_per_country_osrm_haversine(
        x,
        europe,
        shipper_name='Shipper',
        receiver_name='Receiver'):
    ...
    try:
        ...
        df_intersections = europe[LineString(
            line_list).crosses(europe.geometry)]
        if df_intersections.shape[0] > 1:
            df_intersections[['start', 'end']] = df_intersections.apply(
                lambda x: get_segment_boundaries(x, line_list), axis=1)
            df_intersections['distance'] = df_intersections.apply(
                lambda x: get_haversine_distance_lonlat(x['start'], x['end']), axis=1)
            dict_countries = df_intersections.groupby(
                'name')['distance'].sum().to_dict()
