...
try:
    list_terminals = [5]
    df_results_details, df_used, dict_results_combined, dict_results_intermodal, dict_results_multimodal = load_dicts_and_dfs(
        file_path)
    ...
    dict_results_intermodal = {
        i: dict_results_intermodal[i] for i in list_infos}
    dict_results_multimodal = {
        i: dict_results_multimodal[i] for i in list_infos}
    df_results_details = pd.concat(
        [df_results_details_combined, df_results_details_intermodal, df_results_details_multimodal])
    df_used = pd.concat(
        [df_used_combined, df_used_intermodal, df_used_multimodal])
    df_used.to_csv(f"{file_path}/df_used.csv", index=False)
    df_results_details.to_csv(
        f"{file_path}/df_results_details.csv",
        index=False)
...
def co2_combined(_):
    ...
    try:
        ...
        fig_combined = graph_solution_with_radius(df_used[df_used["mode"] == "combined"],
                                                  dict_results_combined["routes railroad"],
                                                  dict_results_combined["terminal allocation"],
                                                  dict_terminals,
                                                  closest_dct,
                                                  list_terminals)
...
def table_combined(_):
    ...
    dff = dff[(dff["mode"] == "combined") & (
        dff["Rail/road"] != "Unimodal road")]
...
def co2_intermodal(_):
    ...
    try:
        ...
        fig_intermodal = graph_solution_with_radius(
            df_used[
                df_used["mode"] == "intermodal"],
            dict_results_intermodal["routes railroad"],
            dict_results_intermodal["terminal allocation"],
            dict_terminals,
            closest_dct,
            list_terminals)
...
def execute_co2_modell(_):
    closest_dct = df_distance_matrix.loc[dc][dict_terminals.keys()].idxmin()
    fig_multimodal = graph_solution_with_radius(df_used[df_used["mode"] == "multimodal"],
                                                dict_results_multimodal["routes railroad"],
                                                dict_results_multimodal["terminal allocation"],
                                                dict_terminals,
                                                closest_dct,
                                                list_terminals)
