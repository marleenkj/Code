...
def zoom_center(lons: tuple=None, lats: tuple=None, projection: str='mercator', width_to_height: float=1.0):
...
def plot_map(df, shipper='all', shipper_lat=50, shipper_long=2):
    zoom, center = zoom_center(pd.concat([df['Receiver longitude'], df['Shipper longitude']]), pd.concat([df['Receiver latitude'], df['Shipper latitude']]))
    fig = go.Figure()
    if shipper == 'all':
        # No shipper selected
    ...
    if shipper == 'all':
        fig.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 40}, mapbox={
            'zoom': 3, "center": {'lat': df['Shipper latitude'].mean(), 'lon': df['Shipper longitude'].mean()}})
    else:
        fig.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 40}, mapbox={
            'zoom': 4, "center": {'lat': df['Shipper latitude'].mean(), 'lon': df['Shipper longitude'].mean()}})
...
def plot_px_bar_client(df, month):
    ...
    df['Pickup time'] = df["Pickup date"].dt.time
    df['Pickup date'] = df["Pickup date"].dt.date
    fig = px.bar(df, x='Pickup date', y="Sender weight (kg)", color="Carrier name", barmode="stack")
    if month is not None:
        fig.update_xaxes(dtick=86400000.0)
    else:
...
def shipper_input(_):
    ...
    shippers = [{"label": value, "value": value} for value in df['DC name'].unique()]
...
def month_input(_):
    months=[{'label': calendar.month_name[value], 'value': value} for value in df['Pickup date'].dt.month.unique()]
...
def table_shipments_columns(_):
    ...
    columns = [
        dict(id='Client name', name='Client name'),
        dict(id='Distance (km)', name='Distance (km)', type='numeric', format=format_float),
        dict(id='Sender weight (kg)', name='Sender weight (kg)', type='numeric', format=format_float),
        dict(id='Number of shippings', name='Number of shippings', type='numeric', format=format_int),
        dict(id='Co2 emissions (g)', name='Co2 emissions (g)', type='numeric', format=format_int),
        dict(id='Co2 BEV (g)', name='Co2 BEV (g)', type='numeric', format=format_int)]
    return columns
...
def table_client_columns(_):
    ...
    columns = [
        dict(id='Client name', name='Client name'),
        dict(id='Pickup date', name='Pickup date'),
        dict(id='DC name', name='DC name)'),
        dict(id='Sender weight (kg)', name='Sender weight (kg)', type='numeric', format=format_float),
        dict(id='Volume (m3)', name='Volume (m3)', type='numeric', format=format_float)]
    return columns
...
def apply_filter_to_datatable(page_current, page_size, filter, sort_by, shipper, month):
    dff = df
...
def get_co2_emissions(shipper, month, distance, filter, selected_rows_indices, rows):
    # format input
    dff = df
    ...
    dff['Co2 emissions new'] = np.where(dff['Distance (km)'] < distance, dff['Co2 BEV (g)'], dff['Co2 emissions (g)'])
