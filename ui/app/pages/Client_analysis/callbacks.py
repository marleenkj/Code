...
import sys
sys.path.append('../..')
from src.plots import graph_solution, graph_solution_direct
...
def correlation_matrix(df_original, closest_dct):
    ...
    df = df[["Distance DC-Client (road)", "Distance DC-C (crow flies)", "Distance DC-C (railroad)",
            "Distance T-T (mainhaul)", "Distance DC-T (endhaul)","GHG Savings Ratio"]]
    df_corr = df.corr()
    fig = go.Figure()
    fig.add_trace(
        go.Heatmap(
...
def client_input(_):
    ...
    clients = [{"label": value, "value": value} for value in dff['client'].unique()]
...
def table_clients_columns(_):
    ...
    columns = [
        dict(id='client', name='Client', type = "text"),
        ...
        dict(id='Recommendation', name='Recommendation', type = "text")]
    return columns
...
def plot_map(client):
    ...
    logger.info(x)
    fig = graph_solution_direct(x, dict_points, dict_terminals, df_distance_matrix)
    return fig
