...
from src.plots import show_clients_dc, show_clients_per_dc_and_both
...
def waterfall_plot(dict_list_values):
    ...
    fig.update_yaxes(
        range=[0, max(max(list(dict_list_values.values()))) + 1000])
        ...
        fig.add_trace(
            go.Scattermapbox(
                lat=[
                    ...
                    dict_terminals[i][1]], mode='markers', marker={
                    'size': 12}, name=f"{i}"))
