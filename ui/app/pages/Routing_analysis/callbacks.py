...
from app import app, df_limited, dict_terminals, df_distance_matrix, dict_results, df_results_details
...
try: 
    ...
    end_date = "2022-01-04"
    df = df[(df["Delivery date"]>"2022-01-03")&(df["Delivery date"]<"2022-01-04")]
    clients = ['C5', 'C13', 'C15', 'C16', 'C22', 'C24', 'C27', 'C28', 'C39']
    df = df[df["Receiver name"].isin(clients)]
    dict_results, df_results_details = co2_modell(df, df_distance_matrix, dict_terminals, start_date, end_date, mode = "intermodal")
...
def shipper_input(_):
    ...
    shippers = ["DC1", "DC2"]
    default_shipper = "DC1"
    closest_dct = df_distance_matrix[default_shipper].loc[dict_terminals.keys()].idxmin()
...
def clients_input(dc):
    terminals = [{"label": value, "value": value} for value in dict_terminals.keys()]
...
def clients_input(shipper, terminal, start_date, end_date):
    ...
    df = df_limited
    logger.info(shipper)
    df = df[(df["Shipper name"]==shipper)&(df["Delivery date"]>start_date)&(df["Delivery date"]<end_date)]
    ...
    clients = [{"label": value, "value": value} for value in df['Receiver name'].unique()]
...
def execute_co2_modell(n_clicks, shipper, terminal, clients, start_date, end_date):
    ...
    logger.info(f"From {start_date} to {end_date} using {shipper} and {clients}")
    ...
    if not n_clicks:
        ...
        # return output_emissions_road, output_emissions_railroad, fig, df_results_details.to_dict('records')
    if n_clicks:
